...
In PHP the include() function is used to load local or remote files. If the include() function is taken from a user-controlled paramater such as GET and if the code does not filter and sanitize the user input, the code is vulrable to remote file inclusion.
